...
class UserDetails(viewsets.ViewSet, ResponseViewMixin):
    ...
    def get_details(self, request, pk=None, user_followed=None, user_logged_in=None):
        ...
        if celebrity and user_logged_in != user.id:
            if (not user.celebrity_user.admin_approval) or (not user.is_active) or \
                    (not user.celebrity_user.star_approved) or user.temp_password:
                return self.jp_error_response(
