...
class StargramzVideoSerializer(CustomModelSerializer):
    ...
    def get_following(self, obj):
        if self.context['request'].user.is_authenticated:
            try:
                CelebrityFollow.objects.get(fan=self.context['request'].user, celebrity_id=obj.stragramz_request.celebrity_id)
                return True
...
class ReactionListingSerializer(serializers.ModelSerializer):
    ...
    def get_share_url(self, obj):
        ...
        return "{}view_reaction/{}".format(web_url, hashids.encode(obj.booking.id))
